...
def generar_pdf_test(nombre_oposicion, preguntas, nombre_base):
    ...
    pdf.cell(200, 10, txt=f"Examen Oficial - {nombre_oposicion}", ln=True, align="C")
    pdf.ln(10)
    ...
    for idx, pregunta in enumerate(preguntas, 1):
        ...
        pdf.multi_cell(0, 10, f"{idx}. {enunciado}")
    ...
    for idx, pregunta in enumerate(preguntas, 1):
        ...
        if not respuesta:
            continue
        pdf.set_font("Arial", size=12)
        pdf.cell(0, 10, f"{idx}. {respuesta}", ln=True)
