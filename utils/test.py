...
def cargar_preguntas_json(nombre_oposicion):
    path = f\"data/test_{nombre_oposicion.lower().replace(' ', '_')}.json\"
...
def generar_test_con_criterio_real(nombre_oposicion):
    ...
    for bloque in criterio[\"bloques_teoricos\"]:
        seleccionadas.extend(random.sample(preguntas_por_bloque[bloque], min(len(preguntas_por_bloque[bloque]), criterio[\"n_teoricas\"] // len(criterio[\"bloques_teoricos\"]))))
    
    for bloque in criterio[\"bloques_ofimatica\"]:
        seleccionadas.extend(random.sample(preguntas_por_bloque[bloque], min(len(preguntas_por_bloque[bloque]), criterio[\"n_ofimatica\"])))
