...
from utils.drive import subir_archivo_a_drive, descargar_archivo_de_drive, CARPETA_TEMARIOS, CARPETA_TEMAS_JSON, CARPETA_TEST_PDF
...
if modo == "📂 Subir nuevo temario":
    ...
    if st.button("📄 Confirmar y registrar temario"):
        if archivo_temario and nombre_oposicion and nombre_temario:
            with st.spinner("Registrando temario..."):
                ...
                for clave in temas_extraidos.keys():
                    st.markdown(f"- {clave}")
    ...
    if not oposiciones:
        st.warning("🚫 No existen oposiciones registradas. Por favor, sube un temario primero.")
    else:
        ...
        if not json_ok:
            ...
            if tipo_test == "Test por temas":
                ...
                if st.button("📝 Generar test"):
                    ruta_test, preguntas = generar_test_desde_tema(opcion, tema_elegido, num_preguntas)
                    st.success("✅ Test generado correctamente.")
                    st.download_button("🔗 Descargar test (JSON)", open(ruta_test, "rb"), file_name=os.path.basename(ruta_test))

            elif tipo_test == "Simulacro examen oficial":
                if st.button("📅 Generar test"):
                    with st.spinner("Generando test oficial..."):
                        ruta_json, ruta_pdf, preguntas = generar_test_examen_completo(opcion, temas_dict)
                        st.success("Test generado correctamente.")
                        st.download_button("🔗 Descargar test PDF", open(ruta_pdf, "rb"), file_name=os.path.basename(ruta_pdf))
